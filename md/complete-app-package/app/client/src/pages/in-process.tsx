...
export default function InProcessTender() {
  ...
  // Fetch tenders with in-process status
  const { data: tenderResponse = { tenders: [], counts: {} }, isLoading } = useQuery({
    queryKey: ['/api/tenders', 'in-process'],
    queryFn: async () => {
      const res = await fetch('/api/tenders?status=in-process');
  ...
  return (
    <>
      <main className="flex-1 p-0 sm:p-0 lg:p-0 bg-gray-100">
        <header className="page-header px-8 pt-8 pb-4">
          <h1 className="text-2xl font-semibold text-gray-800">In-Process Tender</h1>
        </header>
          ...
          <div className="bg-white border-x border-b border-gray-200 rounded-b-lg shadow-sm">
            {isLoading ? (
              ...
              <div className="p-8 text-center">
                <p className="text-gray-500">No in-process tenders available</p>
              </div>
            ) : (
              <div>
