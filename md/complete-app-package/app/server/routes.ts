...
%%EOF`;
  ...
  app.get("/api/checklists/documents/:id/download", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Download request for document ID: ${documentId}`, document);

      // If document has a fileId, get the file from files table
      if (document.fileId) {
        const file = await storage.getFile(document.fileId);
        if (!file) {
          console.error(`File record not found for fileId: ${document.fileId}`);
          return res.status(404).json({ message: "File record not found" });
        }

        console.log(`File record found:`, file);

        // Handle different file path formats - VPS compatibility
        let filePath = file.filePath;
        
        // Ensure absolute path for VPS deployment
        ...
        if (!fs.existsSync(filePath)) {
          ...
          // Try alternative path patterns for VPS
          const alternativePaths = [
            path.join(process.cwd(), 'uploads', path.basename(filePath)),
            path.join('/home', process.env.USER || 'user', 'public_html', 'uploads', path.basename(filePath)),
            path.join('/var/www/html', 'uploads', path.basename(filePath))
          ];
          ...
          if (!foundPath) {
            console.error(`File not found at any location. Tried paths:`, [filePath, ...alternativePaths]);
            return res.status(404).json({ 
              message: "File not found on server",
              details: "The requested document file could not be located on the server."
            });
          }
        ...
        try {
          ...
          // Set appropriate headers for VPS compatibility
          ...
          // VPS-specific headers for better compatibility
          res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
          res.setHeader('Pragma', 'no-cache');
          res.setHeader('Expires', '0');
          
          console.log(`Starting file download: ${filePath} (${stats.size} bytes)`);
          ...
          return res.status(500).json({ 
            ...
            details: "Unable to read the requested file from the server."
          });
        }
        
      } else {
        console.error(`No fileId associated with document ${documentId}`);
        return res.status(404).json({ 
          message: "No file associated with this document",
          details: "This document does not have an associated file for download."
        });
      }
